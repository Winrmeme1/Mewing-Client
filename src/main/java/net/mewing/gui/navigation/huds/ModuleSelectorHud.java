...
public class ModuleSelectorHud extends HudWindow {
	...
	public ModuleSelectorHud() {
		...
		keybindUp = new KeyMapping("key.tabup", GLFW.GLFW_KEY_UP, MewingClient.MEWING_CATEGORY);
		keybindDown = new KeyMapping("key.tabdown", GLFW.GLFW_KEY_DOWN, MewingClient.MEWING_CATEGORY);
		keybindLeft = new KeyMapping("key.tableft", GLFW.GLFW_KEY_LEFT, MewingClient.MEWING_CATEGORY);
		keybindRight = new KeyMapping("key.tabright", GLFW.GLFW_KEY_RIGHT, MewingClient.MEWING_CATEGORY);
	}
	...
	public void update() {
		if (keybindUp.isDown()) {
			...
			if (!isCategoryMenuOpen) {
				...
				if (modules.isEmpty()) {
					for (Module module : MEWING.moduleManager.modules) {
