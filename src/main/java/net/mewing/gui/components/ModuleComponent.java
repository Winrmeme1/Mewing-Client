...
import static net.mewing.utils.render.TextureBank.gear;
...
import net.mewing.gui.navigation.CloseableWindow;
...
public class ModuleComponent extends Component {
	...
	public void update() {
		...
		if (module.isDetectable(MEWING.moduleManager.antiCheat.getValue())) {
	...
	private void onGearClicked(MouseClickEvent e) {
		if (e.button == MouseButton.LEFT && e.action == MouseAction.DOWN) {
			if (lastSettingsTab == null) {
				...
				Mewing.getInstance().guiManager.addWindow(lastSettingsTab, "Modules");
				lastSettingsTab.initialize();
			} else {
				Mewing.getInstance().guiManager.removeWindow(lastSettingsTab, "Modules");
