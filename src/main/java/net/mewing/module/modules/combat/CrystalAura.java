...
import net.mewing.utils.entity.DamageUtils;
...
public class CrystalAura extends Module implements TickListener, Render3DListener {
	...
	private Player getTargetPlayer() {
		...
		for (Player player : players) {
			if (player == MC.player || !targetFriends.getValue() && Mewing.getInstance().friendsList.contains(player))
				continue;
	...
	private void attackCrystal() {
		...
		Iterable<Entity> entities = Mewing.getInstance().entityManager.getEntities();
