...
import net.mewing.command.commands.*;
...
public class CommandManager {
	...
	public static StringSetting PREFIX = StringSetting.builder().id("mewing_prefix").displayName("Prefix")
			.defaultValue(".mewing").build();
		...
		for (Field field : getClass().getDeclaredFields()) {
			if (Command.class.isAssignableFrom(field.getType())) {
				try {
					...
					LogUtils.getLogger().error("Error initializing Mewing commands: " + e.getMessage());
				}
			}
		}
	...
	public void command(String[] commandIn) {
		try {
			...
			if (command == null) {
				sendChatMessage("Invalid Command! Type " + ChatFormatting.LIGHT_PURPLE + ".mewing help" + ChatFormatting.RESET
	...
	public static void sendChatMessage(String message) {
		...
		if (mc.gui != null) {
			mc.gui.getChat().addMessage(Component.nullToEmpty(ChatFormatting.DARK_PURPLE + "[" + ChatFormatting.LIGHT_PURPLE + "Mewing"
