...
import net.mewing.utils.http.HttpUtils;
...
public class MicrosoftAuth {
	...
	static class RefreshTokenHandler implements HttpHandler {
		...
		public void handle(HttpExchange t) throws IOException {
				...
				if (paramSplit.length == 2) {
					if (paramSplit[0].equalsIgnoreCase("code")) {
						...
						try {
							...
							LogUtils.getLogger()
									.info("[Mewing] Error occured while fetching Access Tokens from Microsoft");
			...
			if (!found) {
				LogUtils.getLogger().info("[Mewing] Unable to get Access Tokens from Microsoft");
				onDataReceived.accept(null);
			}
