...
package net.mewing.managers.altmanager;
...
import net.mewing.managers.altmanager.login.MicrosoftAuth;
import net.mewing.mixin.interfaces.IMinecraft;
import net.mewing.utils.system.SystemUtils;
...
/**
 * Class that manages all of the Alt accounts in Mewing.
 */
public class AltManager {
	...
	public void readAlts() {
		try {
			...
			File altFile = new File("mewing_alts.json");
			if (!altFile.exists()) {
	...
	public void saveAlts() {
		try {
			// Finds the file and opens it.
			File altFile = new File("mewing_alts.json");
			LogUtils.getLogger().info("[Mewing] Saving Alts");
			...
			LogUtils.getLogger().error("[Mewing] Failed to save alts");
			exception.printStackTrace();
		}
	}
	...
	public void loginWithAuth(Alt alt, Runnable onComplete) {
		MicrosoftAuth.requestAuthToken((authToken) -> {
			if (authToken != null) {
				...
				LogUtils.getLogger().error("[Mewing] Microsoft authentication failed - no token received.");
			}
			if (onComplete != null) {
				onComplete.run();
			}
